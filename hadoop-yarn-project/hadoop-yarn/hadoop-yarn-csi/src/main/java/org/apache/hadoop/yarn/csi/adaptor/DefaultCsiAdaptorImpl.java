...
public class DefaultCsiAdaptorImpl implements CsiAdaptorPlugin {
    ...
    public void init(String driverName, Configuration conf) throws YarnException {
        ...
        String driverEndpoint = CsiConfigUtils.getCsiDriverEndpoint(driverName, conf);

        LOG.info("DefaultCsiAdaptorImpl init初始化 ==== {}", driverEndpoint);
        LOG.info("This csi-adaptor is configured to contact with" + " the csi-driver {} via gRPC endpoint: {}", driverName, driverEndpoint);
    ...
    public ValidateVolumeCapabilitiesResponse validateVolumeCapacity(ValidateVolumeCapabilitiesRequest request) throws YarnException, IOException {
        ...
        Csi.ValidateVolumeCapabilitiesResponse response = csiClient.validateVolumeCapabilities(req);
