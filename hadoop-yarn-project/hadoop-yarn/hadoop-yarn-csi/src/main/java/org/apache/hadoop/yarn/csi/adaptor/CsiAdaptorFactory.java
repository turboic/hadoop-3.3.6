...
public final class CsiAdaptorFactory {
    ...
    public static CsiAdaptorPlugin getAdaptor(String driverName, Configuration conf) throws YarnException {
        // load configuration
        String configName = YarnConfiguration.NM_CSI_ADAPTOR_PREFIX + driverName + YarnConfiguration.NM_CSI_ADAPTOR_CLASS;

        LOG.info("CsiAdaptorPlugin getAdaptor configName = {}", configName);
        Class<? extends CsiAdaptorPlugin> impl = conf.getClass(configName, DefaultCsiAdaptorImpl.class, CsiAdaptorPlugin.class);
        if (impl == null) {
