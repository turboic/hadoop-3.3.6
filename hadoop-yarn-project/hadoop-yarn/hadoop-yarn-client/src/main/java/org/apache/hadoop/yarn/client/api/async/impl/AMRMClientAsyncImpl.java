...
public class AMRMClientAsyncImpl<T extends ContainerRequest> extends AMRMClientAsync<T> {
    ...
    public AMRMClientAsyncImpl(AMRMClient<T> client, int intervalMs, AbstractCallbackHandler callbackHandler) {
    ...
    public AMRMClientAsyncImpl(AMRMClient<T> client, int intervalMs, CallbackHandler callbackHandler) {
        ...
        heartbeatThread = new HeartbeatThread();
        LOG.info("创建心跳线程heartbeatThread");

        handlerThread = new CallbackHandlerThread();
        LOG.info("创建处理线程handlerThread");

        responseQueue = new LinkedBlockingQueue<Object>();
        LOG.info("创建响应队列responseQueue,队列的最大值是Integer值");
    ...
    protected void serviceStart() throws Exception {
        LOG.info("handlerThread设置成守护线程");
        handlerThread.setDaemon(true);
        handlerThread.start();
        LOG.info("handlerThread线程启动");

        client.start();
        LOG.info("client客户都安启动");

        super.serviceStart();
        LOG.info("super.serviceStart启动");
    }
            ...
            while (true) {
                ...
                try {
                    ...
                    try {
                        ...
                        LOG.info("从响应队列获取对象 === {}", object);
                    } catch (InterruptedException ex) {
                        LOG.debug("Interrupted while waiting for queue,线程出现了异常，要主动进行终端操作", ex);
                    ...
                    TimelineV2Client timelineClient = client.getRegisteredTimelineV2Client();
                    ...
                    if (!updatedNodes.isEmpty()) {
                        LOG.info("需要进行节点更新的处理");
                        handler.onNodesUpdated(updatedNodes);
                    }

                    List<ContainerStatus> completed = response.getCompletedContainersStatuses();
                    if (!completed.isEmpty()) {
                        LOG.info("容器任务已经完成,AMRMClientAsyncImpl调用ApplicationMaster的内部类RMCallbackHandler,进行资源的回调处理");
                        handler.onContainersCompleted(completed);
                        LOG.info("回调类" + handler.getClass().getName());
                        String jar = handler.getClass().getProtectionDomain().getCodeSource().getLocation().toString();
                        LOG.info("容器任务已经完成,LOAD handler from {}", jar);
                    }
                    ...
                    if (!allocated.isEmpty()) {
                        LOG.info("容器allocated,处理参数 === {}", allocated);
                        LOG.info("回调类" + handler.getClass().getName());
                    ...
                    if (preemptionMessage != null) {
                        if (handler instanceof AMRMClientAsync.AbstractCallbackHandler) {
                            ((AMRMClientAsync.AbstractCallbackHandler) handler).onPreemptionMessageReceived(preemptionMessage);
                        }
                    }

                    if (!response.getContainersFromPreviousAttempts().isEmpty()) {
                        if (handler instanceof AMRMClientAsync.AbstractCallbackHandler) {
                            ((AMRMClientAsync.AbstractCallbackHandler) handler).onContainersReceivedFromPreviousAttempts(response.getContainersFromPreviousAttempts());
                        }
                    }
                    List<RejectedSchedulingRequest> rejectedSchedulingRequests = response.getRejectedSchedulingRequests();
                    if (!rejectedSchedulingRequests.isEmpty()) {
                        if (handler instanceof AMRMClientAsync.AbstractCallbackHandler) {
                            ((AMRMClientAsync.AbstractCallbackHandler) handler).onRequestsRejected(rejectedSchedulingRequests);
                        }
                    }
                    progress = handler.getProgress();
                    LOG.info("handler getProgress ===== {}", progress);
                } catch (Throwable ex) {
                    handler.onError(ex);
