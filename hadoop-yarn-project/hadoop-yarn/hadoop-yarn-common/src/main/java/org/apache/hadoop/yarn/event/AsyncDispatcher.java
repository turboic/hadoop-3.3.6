...
public class AsyncDispatcher extends AbstractService implements Dispatcher {
    ...
    public void register(Class<? extends Enum> eventType, EventHandler handler) {
        LOG.info("事件进行注册,事件类型===={}", eventType);
        LOG.info("事件进行注册,事件处理器===={}", handler);
        /* check to see if we have a listener registered */
        LOG.info("Registering " + eventType + " for " + handler.getClass());

        LOG.info("根据事件类型,从缓存中获取处理的事件,事件类型===={}", handler.getClass());

        EventHandler<Event> registeredHandler = (EventHandler<Event>) eventDispatchers.get(eventType);
        if (registeredHandler == null){
            LOG.info("register方法,根据类型【{}】获取EventHandler为空", eventType);
        }
        if (registeredHandler == null) {
            eventDispatchers.put(eventType, handler);
        } else if (!(registeredHandler instanceof MultiListenerHandler)) {
            /* for multiple listeners of an event add the multiple listener handler */
            LOG.info("根据事件类型,从缓存中获取处理的事件,事件类型===={}", "!(registeredHandler instanceof MultiListenerHandler)");
            ...
            LOG.info("根据事件类型,从缓存中获取处理的事件,事件类型===={}", "MultiListenerHandler");
