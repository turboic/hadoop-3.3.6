...
class CleanerTask implements Runnable {
    ...
    public static CleanerTask create(Configuration conf, SCMStore store, CleanerMetrics metrics, Lock cleanerTaskLock) {
        try {
            ...
            LOG.info("创建清理任务 ==sleepTime === {},nestedLevel ======{}", sleepTime, nestedLevel);
    ...
    public void run() {
        if (!this.cleanerTaskLock.tryLock()) {
            ...
            LOG.info("A cleaner task is already running. " + "This scheduled cleaner task will do nothing.");
            return;
        }
