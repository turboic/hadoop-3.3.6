...
public class CleanerService extends CompositeService {
    ...
    private boolean writeGlobalCleanerPidFile() throws YarnException {
        LOG.info("写全局pid文件YarnConfiguration.SHARED_CACHE_ROOT======={}", YarnConfiguration.SHARED_CACHE_ROOT);
        LOG.info("写全局pid文件YarnConfiguration.DEFAULT_SHARED_CACHE_ROOT======={}", YarnConfiguration.SHARED_CACHE_ROOT);
        LOG.info("写全局pid文件GLOBAL_CLEANER_PID======={}", GLOBAL_CLEANER_PID);
