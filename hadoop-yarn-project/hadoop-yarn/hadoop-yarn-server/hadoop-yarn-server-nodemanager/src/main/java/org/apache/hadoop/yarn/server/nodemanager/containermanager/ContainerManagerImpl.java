...
public class ContainerManagerImpl extends CompositeService implements ContainerManager {
    ...
    public ContainerManagerImpl(Context context, ContainerExecutor exec, DeletionService deletionContext, NodeStatusUpdater nodeStatusUpdater, NodeManagerMetrics metrics, LocalDirsHandlerService dirsHandler) {
        ...
        LOG.info("4、创建Auxiliary服务AuxServices");
        ...
        LOG.info("5、创建Container监控服务");

        addService(this.containersMonitor);


        LOG.info("将上面创建的服务注册到dispatcher");
    ...
    protected LogHandler createLogHandler(Configuration conf, Context context, DeletionService deletionService) {
        LOG.info("创建日志操作处理器,Configuration = 【{}】,Context = 【{}】,DeletionService = 【{}】,", conf, context, deletionService);
    ...
    protected AbstractContainersLauncher createContainersLauncher(Context ctxt, ContainerExecutor exec) {
        LOG.info("创建Container加载器,Context = 【{}】,ContainerExecutor = 【{}】,", ctxt, exec);

        LOG.info("创建Container加载器,容器加载类名是 = 【{}】,", YarnConfiguration.NM_CONTAINERS_LAUNCHER_CLASS);
        ...
        try {
            LOG.info("创建Container加载器,配置信息 = 【{}】,", ctxt.getConf());
    ...
    private ContainerManagerApplicationProto buildAppProto(ApplicationId appId, String user, Credentials credentials, Map<ApplicationAccessType, String> appAcls, LogAggregationContext logAggregationContext, FlowContext flowContext) {
    ...
    protected ContainerTokenIdentifier verifyAndGetContainerTokenIdentifier(org.apache.hadoop.yarn.api.records.Token token, ContainerTokenIdentifier containerTokenIdentifier) throws YarnException, InvalidToken {
