...
public class ContainerScheduler extends AbstractService implements EventHandler<ContainerSchedulerEvent> {
    ...
    public ContainerScheduler(Context context, AsyncDispatcher dispatcher, NodeManagerMetrics metrics) {
        this(context, dispatcher, metrics, context.getConf().getInt(YarnConfiguration.NM_OPPORTUNISTIC_CONTAINERS_MAX_QUEUE_LENGTH, YarnConfiguration.DEFAULT_NM_OPPORTUNISTIC_CONTAINERS_MAX_QUEUE_LENGTH));
    ...
    private boolean enqueueContainer(Container container) {
        ...
        LOG.info("enqueueContainer方法,isGuaranteedContainer={}", isGuaranteedContainer);


        boolean isQueued;
        if (isGuaranteedContainer) {
            LOG.info("容器放入队列=【{}】,执行类型是 GUARANTEED", container.getContainerId());
            queuedGuaranteedContainers.put(container.getContainerId(), container);
            isQueued = true;
        } else {
            if (queuedOpportunisticContainers.size() < maxOppQueueLength) {
                LOG.info("队列未满  Opportunistic container {} will be queued at the NM.", container.getContainerId());
                queuedOpportunisticContainers.put(container.getContainerId(), container);
                isQueued = true;
            } else {
                ...
                LOG.info("处理队列已满,需要执行killContainer事件");
                isQueued = false;
            }
        }
        ...
        if (isQueued) {
            try {
                LOG.info("队列可以继续执行数据融入,把容器的id加入到内存缓存【{}】", container.getContainerId());
    ...
    protected void scheduleContainer(Container container) {
        ...
        LOG.info("调度Container方法,isGuaranteedContainer={}", isGuaranteedContainer);
        ...
        if (isGuaranteedContainer) {
            ...
            boolean forceStartGuaranteedContainers = (maxOppQueueLength <= 0);
            LOG.info("调度Container方法,forceStartGuaranteedContainers={}", forceStartGuaranteedContainers);
